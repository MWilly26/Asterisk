...
- **Verify the exact model IDs against build.nvidia.com before hardcoding them.** At time of writing the relevant ones are the Nemotron 3 Nano MoE model (`nvidia/nemotron-3-nano-30b-a3b` or similar) and `nvidia/nemotron-parse`. Put the IDs in `clause/config.py` as constants, never inline, so a single edit fixes a rename.
